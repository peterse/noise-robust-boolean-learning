...
class AutobotConfig:
    ...
    embedding : str = 'embedding' # 'embedding' or 'one_hot'
    tgt_is_causal: bool = False # True: the target sequence is a causal sequence, False: the target sequence is a full sequence
...
class Autobot(nn.Module):
    ...
    def forward(self, token_seq, targets=None, tgt_mask=None):
        """Arguments:
        ...
        elif self.config.embedding == 'one_hot':
            tok_emb = F.one_hot(token_seq, num_classes=self.config.vocab_size).float()
        else:
            raise ValueError(f'Invalid embedding type: {self.config.embedding}')
        ...
        if self.config.tgt_is_causal:
            # if we are using a causal sequence, we must mask the future tokens    
        ...
        x = self.transformer.decoder(x, blank_mem, tgt_mask=tgt_mask, tgt_is_causal=self.config.tgt_is_causal)
